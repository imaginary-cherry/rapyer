...
#### `apipeline(ignore_redis_error=False, use_exising_pipe=False)`
**Type:** `async` context manager
**Parameters:**
...
- `use_exising_pipe` (bool): If `True`, reuses an already active pipeline from an outer context instead of creating a new one. Changes are deferred until the outer pipeline exits.
**Returns:** `AsyncGenerator[Self, None]`
