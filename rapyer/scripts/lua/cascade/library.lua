#!lua name=RAPYER_CASCADE_LIB
-- Redis Functions library for TTL cascade. Registered SERVER-GLOBAL (not
-- per-DB), which is why the library AND function names carry the plan hash --
-- see cascade_names in the Python planner.

-- LIBRARY SCOPE runs ONCE at FUNCTION LOAD, where redis.call is FORBIDDEN. That
-- is exactly why the cascade plan is BAKED into source and cjson.decode'd here
-- as an upvalue instead of GET from a Redis key on every call.
...
local function fk_edges(class_name)
    local entry = CASCADE_PLAN[class_name]
    if entry then
        return entry.fks
    end
    return {}
...
-- The cascade callback. EVERY per-call mutable structure (visited,
-- pending_refresh, refresh_order, stack) and every closure that touches them
-- MUST live INSIDE the callback: hoisting them to library scope would leak
-- state across FCALLs and is a correctness bug. keys[1]=root_key,
...
local function cascade_apply(keys, args)
    ...
    local special_prefix = args[2]
    -- The root's own explicit ttl: applies ONLY to the root's own keys (main +
    -- special), never to any cascade-reached child -- every child still expires
    -- at its owning class's baked-in Meta.ttl.
    local root_ttl = tonumber(args[3])
    ...
    -- tonumber(nil) == nil and nil ~= 0 is true in Lua, so a missing arg keeps
    -- the do_cascade default.
    local do_cascade = tonumber(args[4]) ~= 0

    -- Collect-phase state: the read walk queues every key needing a refresh
    -- (deduped into refresh_order); the write phase below holds the one mutation
    -- (the EXPIRE loop) so the write window stays as small as possible. Every
    -- queued entry carries its OWNING CLASS alongside the key so the write phase
    -- can look up that class's own Meta.ttl -- no single caller-supplied ttl is
    -- ever used, and no GT/NX/XX flag is ever passed to EXPIRE.
    -- `visited` is a best-budget-per-node map, not a boolean set: for each key it
    -- holds the LARGEST budget any path has offered it so far, so a shared node
    -- reached via two paths carrying different finite depth budgets is walked
    -- (and its descendants reached) at the larger of the two, regardless of
    -- which DFS stack frame happens to pop first.
    local visited = {}
    ...
    -- cascade. The per-edge follow/budget decision lives entirely in next_hop; a
    -- node whose own budget is 0 can still follow its explicit-override
    -- (resets_depth_budget) edges (which refresh), matching the field-over-global
    -- override precedence _classify_edge bakes into the plan.
    ...
    -- refreshes its main key / special keys and whether traversal continues from
    -- it. Returns the ordered, deduped {key=, class=} entries to refresh and
    -- mutates no Redis state.
    ...
    -- (returns 0); tally those misses (main vs special) so the caller can observe
    -- a graph that has drifted out of sync -- the root's own keys are never
    -- counted, even if somehow absent.
    local dangling_children_count = 0
    local dangling_special_count = 0
    for _, item in ipairs(plan_refresh_keys()) do
        local ttl = item.is_root and root_ttl or CASCADE_PLAN[item.class].ttl
